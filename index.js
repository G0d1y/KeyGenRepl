...
async function generatePromoCode() {
    try {
        ...
        await saveKeysToMongoDB([promoCode]);
...
async function saveKeysToMongoDB(keys) {
    try {
        saveCount++;
...
async function runPeriodicTask() {
    while (true) {
        try {
            await generateMultiplePromoCodes(99999999999999, 1000);
        } catch (error) {
...
async function main() {
    ...
    const collection = mongoClient.db(DB_NAME).collection("choices");
    const firstDocument = await collection.findOne({});
    GameChoices = firstDocument.choice;
